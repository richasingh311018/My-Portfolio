...
import abgLogo from '../Screenshot 2026-09-13 141137.png';
import ibmLogo from '../Screenshot 2026-09-13 141025.png';
import avinyaLogo from '../Screenshot 2026-09-13 141104.png';
...
function CertificatePreview({ src, alt, rotated = false, cropped = false }) {
  return src.endsWith('.pdf')
    ? <iframe src={src} title={alt} />
    : <img className={`${rotated ? 'certificate-rotated ' : ''}${cropped ? 'ibm-group-cropped' : ''}`} src={src} alt={alt} />;
}
  ...
  <b>def</b> <i>create</i>(self, idea):
    <b>return</b> idea.<u>make_useful</u>()</code></pre>
      ...
      <section className="section-shell certificates-section" id="certificates">
        ...
        <div className="certificate-grid reveal">{[[classAwardPhoto, 'Class 12 Achievement', 'Academic achievement', false], [hindalcoCertificate, 'Hindalco Internship Certificate', 'Hindalco Industries Limited', false], [ibmCertificate, "IBM BOB Hacks '26", 'Grand Finalist', true], [avinyaCertificate, 'IIT Guwahati Avinya', 'IIT Guwahati', true]].map(([image, title, organization, rotated]) => <button className="certificate-card" key={title} onClick={() => setSelectedCertificate({ src: image, alt: title, rotated })}><div className="certificate-image"><div className="photo-button"><CertificatePreview src={image} alt={title} rotated={rotated} /><span className="photo-hint">Click to view <ExternalLink size={13} /></span></div></div><div className="certificate-info"><span className="eyebrow">Certificate</span><h3>{title}</h3><p>{organization}</p><strong>View certificate <ArrowUpRight size={15} /></strong></div></button>)}</div>
      </section>
      ...
      <section className="section-shell experience-section" id="experience">
        ...
        <TiltCard className="experience-card reveal"><div className="experience-mark"><div className="abg-logo" aria-label="IIT Guwahati logo"><img src={avinyaLogo} alt="IIT Guwahati logo" /></div></div><div className="experience-main"><div className="experience-top"><div><span className="eyebrow">IT Internship</span><h3>Hindalco Industries Limited</h3><p>Aditya Birla Group · Renukoot</p></div><span className="placeholder-pill">Excellent</span></div><div className="experience-details"><div><span>Role</span><strong>IT Intern</strong></div><div><span>Duration</span><strong>29 May 2026 – 15 July 2026</strong></div><div><span>Department</span><strong>Information & Technology</strong></div></div><p className="placeholder-copy">Completed an IT internship at Hindalco Industries Limited, working on an AI-Based Predictive Energy Monitoring System.</p><div className="experience-media"><div className="photo-button"><img src={internshipPhoto} alt="Hindalco internship building" onClick={() => setSelectedCertificate({ src: internshipPhoto, alt: 'Hindalco internship building' })} role="button" tabIndex="0" /><span className="photo-hint">Click to view <ExternalLink size={13} /></span></div><div className="photo-button"><img src={internshipCupPhoto} alt="Hindalco internship cup" onClick={() => setSelectedCertificate({ src: internshipCupPhoto, alt: 'Hindalco internship cup' })} role="button" tabIndex="0" /><span className="photo-hint">Click to view <ExternalLink size={13} /></span></div></div></div></TiltCard>
      </section>
      ...
      <section className="section-shell hackathons-section" id="hackathons">
        ...
        <div className="hackathon-grid reveal">        <TiltCard className="hack-card"><div className="hack-top"><span className="hack-number">01</span><span className="finalist">Grand Finalist</span></div><div className="hack-logo ibm-brand" aria-label="IBM Project Bob"><img className="brand-logo ibm-logo" src={ibmLogo} alt="Project Bob IBM logo" /><span className="brand-text"><small>Project Bob</small><b>IBM</b></span></div><h3>IBM BOB Hacks '26</h3><p>Grand Finalist at IBM BOB Hacks '26.</p><div className="hack-footer"><div className="photo-button"><img className="ibm-group-cropped" src={ibmGroupPhoto} alt="IBM BOB Hacks group" onClick={() => setSelectedCertificate({ src: ibmGroupPhoto, alt: 'IBM BOB Hacks group', cropped: true })} role="button" tabIndex="0" /><span className="photo-hint">Click to view <ExternalLink size={13} /></span></div><ArrowUpRight size={17} /></div></TiltCard><TiltCard className="hack-card featured"><div className="hack-top"><span className="hack-number">02</span><span className="finalist">Finalist</span></div><div className="hack-logo avinya-brand" aria-label="Aditya Birla Group logo"><img className="brand-logo avinya-logo" src={abgLogo} alt="Aditya Birla Group logo" /><span className="brand-text"><small>IIT Guwahati</small><b>AVINYA</b></span></div><h3>IIT Guwahati Avinya</h3><p>Finalist at Avinya, IIT Guwahati.</p><div className="hack-footer"><div className="photo-button"><img src={avinyaGroupPhoto} alt="Avinya group" onClick={() => setSelectedCertificate({ src: avinyaGroupPhoto, alt: 'Avinya group' })} role="button" tabIndex="0" /><span className="photo-hint">Click to view <ExternalLink size={13} /></span></div><ArrowUpRight size={17} /></div></TiltCard></div>
      </section>
    ...
    {selectedCertificate && <div className="lightbox" role="dialog" aria-modal="true" onClick={() => setSelectedCertificate(null)}><div className="lightbox-card" onClick={event => event.stopPropagation()}><button onClick={() => setSelectedCertificate(null)} aria-label="Close preview"><X /></button><CertificatePreview src={selectedCertificate.src} alt={selectedCertificate.alt} rotated={selectedCertificate.rotated} cropped={selectedCertificate.cropped} /><p>Preview</p></div></div>}
